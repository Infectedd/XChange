...
public class HuobiAdapters {
	...
	public static Ticker adaptTicker(HuobiTicker huobiTicker, CurrencyPair currencyPair) {
		...
		builder.ask(huobiTicker	.getAsk()
								.getPrice());
		builder.bid(huobiTicker	.getBid()
								.getPrice());
	...
	private static CurrencyPairMetaData adaptPair(HuobiAssetPair pair) {
		// Set the minimum order volume to the minimum price precision as that is effectively the minimum price.
		Integer priceScale = new Integer(pair.getPricePrecision());
		return new CurrencyPairMetaData(null, BigDecimal.ONE.scaleByPowerOfTen(0 - priceScale), null, priceScale);
	}
	...
	public static Wallet adaptWallet(Map<String, HuobiBalanceSum> huobiWallet) {
		...
		for (Map.Entry<String, HuobiBalanceSum> record : huobiWallet.entrySet()) {
			...
			Balance balance = new Balance(currency, record	.getValue()
															.getTotal(), record	.getValue()
																				.getAvailable(), record	.getValue()
																										.getFrozen());
	...
	private static Order adaptOrder(HuobiOrder openOrder) {
		...
		if (openOrder.isLimit()) {
			order = new LimitOrder(	orderType, openOrder.getAmount(), currencyPair, String.valueOf(openOrder.getId()), openOrder
																																.getCreatedAt(),
									openOrder.getPrice());
		}
		if (order != null) {
	...
	private static OrderType adaptOrderType(String orderType) {
		return orderType.substring(1, 3)
						.equals("buy") ? OrderType.BID : OrderType.ASK;
	}
